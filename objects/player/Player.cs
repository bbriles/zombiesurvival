...
///   2. Add a Camera3D as a child and assign it to the `Camera` export.
///   3. (Optional) Add a CollisionShape3D — a CapsuleShape3D works well.
///   4. Define the Input Actions below in Project → Input Map, or rely on
///      the built-in key bindings set up in _Ready().
/// </summary>
public partial class Player : CharacterBody3D
{
    ...
    [Export] public Marker3D MuzzlePoint;
    ...
    private void Shoot()
    {
        if (BulletScene == null || MuzzlePoint == null) return;

        var bullet = BulletScene.Instantiate<Bullet>();
        GetTree().Root.AddChild(bullet);

        // Use the muzzle marker's world transform so the bullet spawns at the
        // barrel tip and flies in whichever direction the marker is pointing.
        bullet.GlobalTransform = MuzzlePoint.GlobalTransform;
    }
}
